/*
package com.mekcone.incrud.service.generators.frontend;
...
public class ApiDocumentGenerator {
    public ApiDocumentGenerator(ProjectModel projectModel, String path) {

        String html = "<!--" + ProjectBuilder.getDescription() + "-->\n" + document(html(
                head(
                        title(projectModel.getArtifactId().capitalized() + " API"),
                        meta().withCharset("UTF-8"),
                        styleWithInlineFile( System.getProperty("user.dir") + "/templates/api.css")
                ),
                body(
                        div(
                                h1(projectModel.getArtifactId().capitalized() + " API")
                        ).withClass("header"),
                        each(projectModel.getTables(), table ->
                            div(
                                    h2(table.getName().capitalizedCamelStyle()),

                                    span("POST").withClass("PostSpan"),
                                    h3("/api/" + projectModel.getArtifactId().camelStyle() + "/" + table.getName().camelStyle()),
                                    ...
                                    span("GET").withClass("GetSpan"),
                                    h3("/api/" + projectModel.getArtifactId().camelStyle()  + "/" + table.getName().camelStyle() + "/{" + table.getPrimaryKey().camelStyle() + "}"),
                                    p("Content-type :  application/json"),
                                    p("Retrieve the list of all " + table.getName().camelStyle() + " items").withClass("description"),
...
/*each (table.getColumns(), column ->
        ...
        FileUtil.write(path + "/api.html", html);
        LogUtil.info("Output API web page file \"" + path + "/api.html" + "\"");
    }
