...
public class ProjectServiceOld {
    ...
    public void save(ProjectOld project) {
        mongoTemplate.save(project);
    }

    public ProjectOld query(String projectId) {
        ProjectOld project = mongoTemplate.findById(projectId, ProjectOld.class);
        if (project != null && project.getDataAccess() != null) {
            project.getDataAccess().setDataStoreOlds(dataStoreService.queryList(projectId));
        }
        return project;
    }

    public ResponsePage<ProjectOld> queryPage(int pageNo, int pageSize, String search) {
        ...
        long count = mongoTemplate.count(query, ProjectOld.class);
        List<ProjectOld> projects = mongoTemplate.find(query.with(pageable), ProjectOld.class);
        return new ResponsePage<>(new PageImpl<>(projects, pageable, count));
    }

    public void delete(String projectId) {
        ProjectOld project = query(projectId);
        if (project != null) {
    ...
    public void saveSolution(String projectId, ProjectOld.DataAccess.Solution solution) {
        ProjectOld project = query(projectId);
        if (project != null) {
            if (project.getDataAccess() == null) {
                project.setDataAccess(new ProjectOld.DataAccess());
            }
    ...
    public Map<String, Integer> overview(String projectId) {
        ...
        if (project != null) {
            ...
            if (project.getDataAccess() != null) {
                if (project.getDataAccess().getDataStoreOlds() != null) {
                    dataStoreCount = project.getDataAccess().getDataStoreOlds().size();
                }
            }
