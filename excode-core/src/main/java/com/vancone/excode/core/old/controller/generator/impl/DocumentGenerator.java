...
import com.vancone.excode.core.old.controller.extmgr.document.MarkdownExtensionManager;
import com.vancone.excode.core.old.controller.extmgr.document.PdfExtensionManager;
...
import com.vancone.excode.core.old.model.module.impl.DocumentModule;
import com.vancone.excode.core.old.model.project.ProjectOld;
import com.vancone.excode.core.util.DataTypeConverter;
...
public class DocumentGenerator extends Generator {
    ...
    public DocumentGenerator(ProjectOld projectOld) {
        ...
        for (String languageType : projectOld.getLanguages()) {
    ...
    public void generatePdf(String languageType) {
        try {
            ...
            PdfWriter pdfWriter = PdfWriter.getInstance(document, new FileOutputStream(outputPath + projectOld.getName().getValue(languageType) + LangUtil.separator(languageType) + LangUtil.get(languageType, "api_document") + ".pdf"));
            ...
            String title = projectOld.getName().getValue(languageType) + "\n" + LangUtil.get(languageType, "api_document");
            ...
            font.setSize(14);
            paragraph.setFont(font);
            chunk = new Chunk("v" + projectOld.getVersion());
            paragraph.add(chunk);
            document.add(paragraph);
            ...
            Database database = projectOld.getModuleSet().getDatasourceModule().getRelationalDatabase().getDatabases().get(0);
            for (int i = 0; i < database.getTables().size(); i++) {
                ...
                List<DocumentModule.Keyword> keywords = projectOld.getModuleSet().getDocumentModule().getKeywords();
