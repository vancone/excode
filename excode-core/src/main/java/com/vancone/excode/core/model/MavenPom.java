...
public class MavenPom {
    ...
    public MavenPom(ProjectOld projectOld) {
        groupId = projectOld.getGroupId();
        artifactId = projectOld.getArtifactId();
        version = projectOld.getVersion();
        name = projectOld.getName().getDefaultValue();
        description = projectOld.getDescription().getDefaultValue();

        addDependenciesOld("default");
    }
...
//        addDependencies("default");
    ...
    public void addDependencyByLabel(String label) {
        MongoTemplate mongoTemplate = TemplateFactory.getMongoTemplate();
        List<Dependency> dependencies = mongoTemplate.find(Query.query(Criteria.where("label").is(label)), Dependency.class);
        if (!dependencies.isEmpty()) {
            this.dependencies.addAll(dependencies);
        }
    }

    public void addDependenciesOld(String pomFileName) {
