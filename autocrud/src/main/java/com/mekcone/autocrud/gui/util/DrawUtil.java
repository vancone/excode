package com.mekcone.autocrud.gui.util;
...
public class DrawUtil {
    ...
    public static void addDrawFunc(Stage stage, Node root) {
        ...
        root.setOnMouseDragged((MouseEvent event) -> {
            Logger.debug("addDrawFunc Called");
            double x = event.getSceneX();
            double y = event.getSceneY();
    ...
    public static void addDrawFunc2(Pane root) {

        root.setOnMouseExited((MouseEvent event) -> {
            Logger.debug("mouse exit");
                }
        ...
        root.setOnMouseDragged((MouseEvent event) -> {
            Logger.debug("addDrawFunc2 Called");
            double x = event.getSceneX();
            double y = event.getSceneY();
            // 保存窗口改变后的x、y坐标和宽度、高度，用于预判是否会小于最小宽度、最小高度
...
//            root.setWidth(nextWidth);
            root.setPrefWidth(nextWidth - 30);  // - LeftBar's width
